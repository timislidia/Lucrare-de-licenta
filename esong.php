...
<section id="meniu">
	...
	<div class="fbuttons">
		...
		$resultSet = $mysqli->query("SELECT id_resursa FROM resurse WHERE autor='Unknown' ");
        $numRows = $resultSet->num_rows;
        $resultSet = $mysqli->query("SELECT * FROM resurse WHERE autor='Unknown' ");

		while($row= mysqli_fetch_array($resultSet)) {
			echo '<span><br>' . $row["titlu"] . '</span>' ; $nbsp;
			echo '<br>'; ?>
